...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    public static String DATE_FORMAT_DATEONLY1 = "yyyyMMdd"; // 年/月/日
    public static String DATE_FORMAT_DATETIME = "yyyy-MM-dd HH:mm:ss"; // 年/月/日
    public static String DATE_FORMAT_DATETIME_NO_SS = "yyyy-MM-dd HH:mm"; // 年/月/日
    public static String DATE_FORMAT_DATETIME_WITH_MSEC = "yyyy-MM-dd HH:mm:ss.SSS"; // 年/月/日

    public static SimpleDateFormat sdfDateTime = new SimpleDateFormat(DateUtils.DATE_FORMAT_DATETIME);

    public static SimpleDateFormat sdfDateTimeWithSSS = new SimpleDateFormat(DateUtils.DATE_FORMAT_DATETIME_WITH_MSEC);
    ...
    public static final SimpleDateFormat DATE_FORMAT_COMMON = new SimpleDateFormat("yyyy-MM-dd HH:mm");
    ...
    //函数的参数有小时、分、秒、相比今天的日期，今天就输入0，明天输入1，昨天输入-1，以此类推
    ...
    public static String getSysDateTimeWithSSSString() {
        return toString(new Date(System.currentTimeMillis()), DateUtils.sdfDateTimeWithSSS);
    }
...
//        sdf.setTimeZone(TimeZone.getTimeZone("CST"));
    ...
    public static String getLastWeekDay(Date date) {
        ...
        return DateUtils.formatDate(calendar.getTime(),DATE_FORMAT_DATEONLY);
    }
     ...
     * @return
     */
    public static String getWeekDate(Integer weekday, Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        // 取当前日期是星期几(week:星期几)
        int week = calendar.get(Calendar.DAY_OF_WEEK);
        // 如果传入的星期大于等于今天的星期,则日志往后取（weekDay - week）天
        // 否则日期加上7再减去（week - weekDay）天
        if (weekday >= week) {
            calendar.add(Calendar.DATE, (weekday + 1 - week));
        } else {
            calendar.add(Calendar.DATE, 7 - week + weekday + 1);
        }
     ...
     * 当前时间的前几天，并且以例如2013/12/09 00:00:00 形式输出
     */
    public static String subDaysByDate(int days,String dateParam) throws Exception{
        Date date = addDay(parseDate(dateParam,DATE_FORMAT_DATEONLY), -days);
        return getReqDate(date);
    }
    ...
    public static String formatDateWithSSS(Date date) {
        if (date == null) {
            return "";
        }
    ...
    public static boolean isInBetweenTimes3(Date nowTime,String startTime, String endTime) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String time = sdf.format(nowTime);
        if (time.compareTo(startTime) >= 0 && time.compareTo(endTime) <= 0) {
            return true;
     ...
     * @return
     */
    public static Date getDateByStr(String dateStr, SimpleDateFormat format) {
        try {
     ...
     * @return
     */
    public static Date getFirstDayOfMonth(Date date,int ...args) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        calendar.set(Calendar.HOUR_OF_DAY, 0);
        calendar.set(Calendar.MINUTE, 0);
    ...
    public static int getWeekNum(){
        ...
        return lastDate.get(Calendar.WEEK_OF_YEAR);
    }
     ...
     * @param dayOfWeek {@link Calendar} 周日:1 周一:2 ....周六:7
     * @param weekOffset  周偏移，上周为-1，本周为0，下周为1，以此类推
     * @return
     */
    public static Date getDayOfWeek(int firstDayOfWeek,int dayOfWeek,int weekOffset,int ...args){
        ...
        if(args.length==1){
            date.set(Calendar.HOUR_OF_DAY,args[0]);
        }
        if(args.length==2){
            date.set(Calendar.MINUTE,args[1]);
        }
        return date.getTime();
    }

    public static Date getDayOfWeek(Date date2,int firstDayOfWeek,int dayOfWeek,int weekOffset,int ...args){
        if(dayOfWeek>Calendar.SATURDAY || dayOfWeek<Calendar.SUNDAY){
            return null;
        }
        if(firstDayOfWeek>Calendar.SATURDAY || firstDayOfWeek < Calendar.SUNDAY){
    ...
    public static String getInteractionTime(Date date){
        //当天
        if (DateUtils.compareDate(date, new Date(), Calendar.DATE) == 0) {
            ...
            if(DateUtils.getDateDiff(date, new Date())<= CommonConstant.FIVE){
                ...
                return DateUtils.formatDate(date,DATE_FORMAT_DATETIME_NO_SS);
            }
        }
    }


    public static double comDouble(char type, int scale, Double... vs) {
        if (vs != null && vs.length != 0) {
            BigDecimal ret = null;
            if (vs[0] == null) {
                ret = BigDecimal.ZERO;
            } else {
                ret = new BigDecimal(vs[0]);
            }
                ...
                switch(type) {
                    case '*':
                        for(i = 1; i < vs.length; ++i) {
                            if (vs[i] == null) {
                                throw new RuntimeException("value[" + i + "] can not be null");
                            }

                            ret = ret.multiply(new BigDecimal(vs[i]));
                        }
                    ...
                    case '+':
                        for(i = 1; i < vs.length; ++i) {
                            ...
                            ret = ret.add(new BigDecimal(vs[i]));
                        }
                    case ',':
                    case '.':
                    default:
                    ...
                    case '-':
                        for(i = 1; i < vs.length; ++i) {
                            ...
                            ret = ret.subtract(new BigDecimal(vs[i]));
                        }
                    ...
                    case '/':
                        for(i = 1; i < vs.length; ++i) {
                            ...
                            ret = ret.divide(new BigDecimal(vs[i]), 10, 6);
                        }
                }
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        FilePrefixEnum filePrefixEnum = FilePrefixEnum.valueOf("CUSTOMER111");
        switch(filePrefixEnum){
            case CUSTOMER :
...
//        String aa = "2019-08-12";
//
//        String bb = "2019-07-12";
//
...
//        DateUtils.StringToDate("2019-09-02 17:00:00",DateUtils.DATE_FORMAT_DATEONLY).getTime();
...
//
     ...
     * @authur 杨兴梅
     */
    public static List<String> getMonths(Integer date) {
        Calendar calendar = Calendar.getInstance();
        if (date.equals(calendar.get(Calendar.YEAR))) {
            return haveMonths(calendar.get(Calendar.MONTH) + 1, date);
        } else {
            return haveMonths(12, date);
        }
    }

    static List<String> haveMonths(Integer maxMonth, Integer date) {
     ...
     * @authur 杨兴梅
     */
    public static Date getStart(String formatStr) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
     ...
     * @authur 杨兴梅
     */
    public static String getSegmentation(Date date) {
        String segmentation = "";
        GregorianCalendar ca = new GregorianCalendar();
        ca.setTime(date);
        int time = ca.get(GregorianCalendar.HOUR_OF_DAY);
        if (time < 12) {
            segmentation = "早上";
        } else if (time < 18) {
            segmentation = "下午";
    ...
    public static Integer getHours(Date date) {
        GregorianCalendar ca = new GregorianCalendar();
        ca.setTime(date);
        return ca.get(GregorianCalendar.HOUR_OF_DAY);
    }
    ...
    public static Integer getMinutes(Date date) {
        GregorianCalendar ca = new GregorianCalendar();
        ca.setTime(date);
        return ca.get(GregorianCalendar.MINUTE);
    }
     ...
     *
     * @param startDate
     * @return 传入的时间和当前时间的差值
     */
    public static Integer calLastedTime(Date startDate) {
        return DateUtils.calLastedTime(startDate, new Date());
    }
     ...
     * @return 传入的时间和当前时间的差值
     */
    public static String calLastedFormatTime(Date startDate, Date endDate) {
        long a = startDate.getTime();
        long b = endDate.getTime();
        int c = (int) ((b - a) / 1000);
        int second = c / 60;
        int minute = c % 60;
        return second + "分" + minute + "秒";
    }
    ...
    public static boolean isSameDay(long time1, long time2) {
        ...
        return t1.get(Calendar.YEAR) == t2.get(Calendar.YEAR) &&
                t1.get(Calendar.DAY_OF_YEAR) == t2.get(Calendar.DAY_OF_YEAR);
    }
